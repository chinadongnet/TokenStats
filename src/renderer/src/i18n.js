...
const en = {
  ...
  // cloud sync (tokenstat-web)
  'set.cloudSection': 'Cloud sync',
  'set.cloudEnabled': 'Sync usage to the cloud',
  'set.cloudEndpoint': 'Endpoint',
  'set.cloudKey': 'TokenStat Key',
  'set.cloudKeyHint': 'Create a key on the website (one per device): {url}/keys',
  'set.cloudInterval': 'Sync every (min)',
  'set.cloudTest': 'Test connection',
  'set.cloudTestOk': 'OK — bound to {username} / {deviceName} ({rows} rows in cloud)',
  'set.cloudTestFail': 'Connection failed: {error}',
  'set.cloudSyncNow': 'Sync now',
  'set.cloudFullResync': 'Full resync',
  'set.cloudSyncing': 'Syncing…',
  'set.cloudNever': 'Never synced yet',
  'set.cloudLastSync': 'Last synced {time}',
  'set.cloudLastError': 'Last error: {error}',
  'set.cloudSaved': 'Saved — sync runs in the background.',
  'set.cloudHint': 'Pushes hourly per-CLI/model token totals and the popup status to your tokenstat-web site. No project paths, session content or key aliases ever leave this machine.',
  'set.save': 'Save',
  'set.modelRetired': 'retired',
  // updates (GitHub releases)
  'set.version': 'Version',
  'set.versionLine': 'v{ver} · built {built}',
  'set.checkUpdate': 'Check for updates',
  'set.checking': 'Checking…',
  'set.upToDate': 'Up to date — v{ver} is the latest release.',
  'set.updateAvailable': 'v{ver} is available (you have v{cur}).',
  'set.updateCheckFailed': "Couldn't check GitHub: {error}",
  'set.download': 'Download & install',
  'set.downloading': 'Downloading… {pct}%',
  'set.downloadFailed': 'Download failed: {error}',
  'set.installReady': 'Downloaded. Installing will close TokenStats and reopen it on the new version.',
  'set.installNow': 'Install & restart',
  'set.installFailed': 'Install failed: {error}',
  'set.releaseNotes': 'Release notes',
  'set.openReleases': 'Open releases page',
  'set.updateDevBuild': 'Running an unpackaged dev build — install manually from the releases page.',
  'set.updateNoAsset': 'That release has no installer attached — download it from the releases page.',
  'set.updateHint': 'TokenStats updates from its GitHub releases: {repo}',
  ...
  'set.currentCycle': 'Current billing cycle',
  'set.subscriptionFee': 'Subscription fee',
  'set.tokenUsageCost': 'Token usage cost',
  'set.cycleTokens': '{tokens} tokens used this cycle',
  'set.cycleValueRatio': 'usage value is {ratio}% of fee',
  'set.noFeeRatio': 'no subscription fee to compare',
  ...
  // dedicated subscription-cycle window
  'cyc.title': 'Subscription cycles',
  'cyc.subtitle': 'Monthly fee vs actual token usage value',
  'cyc.empty': 'No subscription cycles yet. Add a token plan in Settings first.',
  'cyc.active': 'Active',
  'cyc.ended': 'Ended',
  'cyc.currentCycle': 'Current monthly cycle',
  'cyc.lastCycle': 'Last monthly cycle',
  'cyc.tokensUsed': 'Tokens used',
  'cyc.tokens': 'tokens',
  'cyc.tokenCost': 'Token usage cost',
  'cyc.subscriptionFee': 'Subscription fee',
  'cyc.paygEstimate': 'pay-as-you-go estimate',
  'cyc.forCycle': 'for this billing cycle',
  'cyc.valueRatio': 'Value ratio',
  'cyc.costDivFee': 'usage cost ÷ subscription fee',
  'cyc.noFee': 'no fee to compare',
  'cyc.peakCycle': 'Highest-token cycle',
  'cyc.activePlans': 'Active plans',
  'cyc.currentCycles': 'with a current billing cycle',
  'cyc.totalTokens': 'Current-cycle tokens',
  'cyc.activeCycleUsage': 'across active subscriptions',
  'cyc.totalCost': 'Current usage value',
  'cyc.overallValue': 'Overall value ratio',
  'cyc.footer': 'TokenStats v{ver} · built {built}',
  // request log
...
const zh = {
  ...
  // 云同步（tokenstat-web）
  'set.cloudSection': '云同步',
  'set.cloudEnabled': '同步用量到云端',
  'set.cloudEndpoint': 'Endpoint',
  'set.cloudKey': 'TokenStat Key',
  'set.cloudKeyHint': '在网站上创建 Key（每台设备一个）：{url}/keys',
  'set.cloudInterval': '同步间隔（分钟）',
  'set.cloudTest': '测试连接',
  'set.cloudTestOk': '连接成功——绑定到 {username} / {deviceName}（云端已有 {rows} 行）',
  'set.cloudTestFail': '连接失败：{error}',
  'set.cloudSyncNow': '立即同步',
  'set.cloudFullResync': '全量重传',
  'set.cloudSyncing': '同步中…',
  'set.cloudNever': '尚未同步',
  'set.cloudLastSync': '上次同步 {time}',
  'set.cloudLastError': '上次错误：{error}',
  'set.cloudSaved': '已保存——同步将在后台进行。',
  'set.cloudHint': '推送每小时的按 CLI/模型 token 汇总和弹窗状态到你的 tokenstat-web 站点。项目路径、会话内容、Key 别名等绝不会离开本机。',
  'set.save': '保存',
  'set.modelRetired': '已下架',
  // 更新（GitHub releases）
  'set.version': '版本',
  'set.versionLine': 'v{ver} · 构建于 {built}',
  'set.checkUpdate': '检查更新',
  'set.checking': '检查中…',
  'set.upToDate': '已是最新——v{ver} 就是最新版本。',
  'set.updateAvailable': '发现新版本 v{ver}（当前 v{cur}）。',
  'set.updateCheckFailed': '无法连接 GitHub：{error}',
  'set.download': '下载并安装',
  'set.downloading': '下载中… {pct}%',
  'set.downloadFailed': '下载失败：{error}',
  'set.installReady': '下载完成。安装会关闭 TokenStats，并以新版本重新启动。',
  'set.installNow': '安装并重启',
  'set.installFailed': '安装失败：{error}',
  'set.releaseNotes': '更新说明',
  'set.openReleases': '打开发布页',
  'set.updateDevBuild': '当前是未打包的开发版——请到发布页手动安装。',
  'set.updateNoAsset': '该版本没有附带安装包——请到发布页下载。',
  'set.updateHint': 'TokenStats 通过 GitHub releases 更新：{repo}',
  'set.tokenPlans': '订阅计划',
  'set.addPlan': '+ 添加计划',
  ...
  'set.currentCycle': '当前月度订阅周期',
  'set.subscriptionFee': '订阅费用',
  'set.tokenUsageCost': 'Token 用量折算费用',
  'set.cycleTokens': '本周期已使用 {tokens} Token',
  'set.cycleValueRatio': '用量价值为订阅费的 {ratio}%',
  'set.noFeeRatio': '无订阅费用可供对比',
  ...
  // 独立订阅周期窗口
  'cyc.title': '订阅周期',
  'cyc.subtitle': '月度订阅费与实际 Token 用量价值对比',
  'cyc.empty': '还没有订阅周期，请先在设置中添加订阅计划。',
  'cyc.active': '生效中',
  'cyc.ended': '已停用',
  'cyc.currentCycle': '当前月度周期',
  'cyc.lastCycle': '最后月度周期',
  'cyc.tokensUsed': '已使用 Token',
  'cyc.tokens': 'Token',
  'cyc.tokenCost': 'Token 用量折算费用',
  'cyc.subscriptionFee': '订阅费用',
  'cyc.paygEstimate': '按量计费估算',
  'cyc.forCycle': '当前计费周期',
  'cyc.valueRatio': '价值比',
  'cyc.costDivFee': '用量费用 ÷ 订阅费用',
  'cyc.noFee': '无订阅费用可供对比',
  'cyc.peakCycle': '单周期最高 Token 用量',
  'cyc.activePlans': 'Active 订阅',
  'cyc.currentCycles': '具有当前计费周期',
  'cyc.totalTokens': '当前周期 Token',
  'cyc.activeCycleUsage': '全部 Active 订阅合计',
  'cyc.totalCost': '当前用量价值',
  'cyc.overallValue': '总体价值比',
  'cyc.footer': 'TokenStats v{ver} · 构建于 {built}',
  // request log
